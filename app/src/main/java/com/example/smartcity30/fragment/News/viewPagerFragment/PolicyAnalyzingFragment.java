...
public class PolicyAnalyzingFragment extends Fragment {
    ...
    public RecyclerView rv_policy_analyzing_news;
    public String BASE_URL = "http://124.93.196.45:10001";
    public List<NewsDetailsResult.RowsBean> newsDetailsDataList = new ArrayList<>();

    public PolicyAnalyzingFragment(int newsTypeIndex) {
        this.newsTypeIndex = newsTypeIndex;
    }

    public static PolicyAnalyzingFragment newInstance(int newsTypeIndex) {
        return new PolicyAnalyzingFragment(newsTypeIndex);
    }

    private void initView() {
        rv_policy_analyzing_news = view.findViewById(R.id.rv_policy_analyzing_news);
        rv_policy_analyzing_news.setLayoutManager(new LinearLayoutManager(getActivity(), LinearLayoutManager.VERTICAL, false));
        rv_policy_analyzing_news.setAdapter(new NewsFragmentNewsListAdapter(newsDetailsDataList, requireActivity()));
    }
